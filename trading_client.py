...
from config import POLYGON_API_KEY, FINANCIAL_PREP_API_KEY, API_KEY, API_SECRET, mongo_url
...
def initialize_clients():
    ...
    return client, trading_client, data_client, mongo_client


def update_market_status(client, mongo_client):
...
def fetch_tickers_and_coefficients(mongo_client):
    ...
    return ndaq_tickers, strategy_to_coefficient


def execute_trading_logic(trading_client, mongo_client, ndaq_tickers, strategy_to_coefficient):
...
def main():
    client, trading_client, data_client, mongo_client = initialize_clients()
    ndaq_tickers = []
    early_hour_first_iteration = True
    post_hour_first_iteration = True

    while True:
        status = update_market_status(client, mongo_client)

        if status == "open":
            logging.info("Market is open. Waiting for 60 seconds.")
            if not ndaq_tickers:
                ndaq_tickers, strategy_to_coefficient = fetch_tickers_and_coefficients(mongo_client)
                early_hour_first_iteration = False
                post_hour_first_iteration = True
            execute_trading_logic(trading_client, mongo_client, ndaq_tickers, strategy_to_coefficient)
